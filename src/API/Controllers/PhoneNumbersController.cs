...
namespace PhoneNumberFormatter.API.Controllers
{
    ...
    public class PhoneNumbersController : BaseApiController
    {
        ...
        [Route("prettify")]
        public IActionResult PrettifyE164([FromBody] string phoneNumber)
        {
            try
            {
                // Validate the request
                if (string.IsNullOrWhiteSpace(phoneNumber))
                    throw new ArgumentException($"{nameof(phoneNumber)} must have a value.");

                Logger.LogTrace($"Prettifying ohone number: {phoneNumber}");
